...
from machine import Pin, SDCard, Timer
import board
import digitalio
import busio
...
while not self.hwspi.try_lock():
    pass
spi.configure(baudrate=spi_freq, polarity=0, phase=0)
...
def autorepeat( timer):
  ...
  irq_handler(25) # catch stale IRQ
...
def change_file():
  ...
  if filename:
    if filename.endswith(".bit"):
      ...
      irq_handler(25) # handle stuck IRQ
...
init_fb()
read_dir()
irq_handler(25) # service eventual pending/stale IRQ
# activate IRQ service
irq_handler_ref = irq_handler # allocation happens here
spi_request = Pin(25, Pin.IN, Pin.PULL_UP)
spi_request.irq(trigger=Pin.IRQ_FALLING, handler=irq_handler_ref)
